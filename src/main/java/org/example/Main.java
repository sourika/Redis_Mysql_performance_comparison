...
//        properties.put(Environment.USE_SQL_COMMENTS, "true");
    ...
    public static void main(String[] args) {
        ...
        //выбираем случайных 10 id городов (существующие в БД id)
    ...
    private List<City> fetchData(Main main) {
        ...
        try (Session session = getSessionFactory().getCurrentSession()) {
            ...
            List<Country> countries = main.getCountryDAO().getAll();
            int count = Math.toIntExact(main.getCityDAO().getTotalCount());
            int step = 500;

            for (int i = 0; i < count; i += step) {
                cities.addAll(main.getCityDAO().getItems(i, step));
            }
            transaction.commit();
